...
const Dashboard = () => {
	...
	useEffect(() => {
		...
		const currentUserId =
			JSON.parse(localStorage.getItem('userId')) === null ? 25 : JSON.parse(localStorage.getItem('userId'));
	...
	return (
		<div>
								...
								<div className="input-group">
									<div className="input-group-prepend">
										<div className="input-group">
											<select
												id="album"
												onChange={handleInputChangeAlbum}
												className="form-control"
											>
												<option key={'index'}>{'Seleccionar Album'}</option>
												{albums.map((al, index) => <option key={index}>{al.name}</option>)}
											</select>
										</div>
									</div>
								</div>
			...
			<div className="row row-cols-1 row-cols-md-5 g-4 container-fluid">
				{fotos.map((foto, index) => <Card key={index} foto={foto} index={index} />)}
			</div>
		</div>
	);
};
